...
__global__ void forward_face_index_map_cuda_kernel_1(
    ...
    /* find indices of blocks where the face potentially has an overlap */
    const int offset = 0;
    // const int x_min = max(floor(p[0][0] / block_size)-offset, 0.0);
    // const int x_max = min(ceil(p[2][0] / block_size)+offset, is / block_size -1.);
    // const int y_min = max(floor(min(min(p[0][1], p[1][1]), p[2][1]) / block_size)-offset, 0.);
    ...
    for (int x = x_min / block_size; x <= x_max/block_size; x++) {
        for (int y = y_min/block_size; y <= y_max/block_size; y++) {
            int ind = atomicInc((unsigned int*)&face_list[bn*block_image_size*block_image_size*buffer_size + y*block_image_size*buffer_size + x*buffer_size], 2*buffer_size);
	    if (ind <= buffer_size-1) {
            	face_list[bn*block_image_size*block_image_size*buffer_size + y*block_image_size*buffer_size + x*buffer_size + ind+1] = fn;
	    }
        }
    }
}
...
__global__ void forward_face_index_map_cuda_kernel_2(
    ...
    const int bn = blockIdx.z * blockDim.z + threadIdx.z;
    if ( (xi >= image_size) || (yi >= image_size) || (bn >= batch_size) ) {
        return;
    }
    ...
    const int block_image_size = image_size / blockDim.x;
    const int block_offset = bn*block_image_size*block_image_size*buffer_size + block_y*block_image_size*buffer_size + block_x*buffer_size;
    const int img_offset = bn*image_size*image_size + yi*image_size + xi;
    const int num_block_faces = face_list[block_offset];
    for (int i = 0; i < num_block_faces; i++) {
        int fn = face_list[block_offset + i + 1];
        /* go to next face */
        const scalar_t *face = &faces[9*bn*num_faces + 9*fn];
        const scalar_t *face_inv = &faces_inv[9*bn*num_faces + 9*fn];
    ...
    // // /* set to global memory */
    if (0 <= face_index_min) {
        depth_map[bn*image_size*image_size + yi*image_size + xi] = depth_min;
        face_index_map[bn*image_size*image_size + yi*image_size + xi] = face_index_min;
        if (return_visibility) {
            atomicMax(&face_visibility[bn*num_faces + face_index_min], 1);
        }
        for (int k = 0; k < 3; k++) {
            weight_map[bn*image_size*image_size*3 + yi*image_size*3 + xi*3 +k] = weight_min[k];
        }
        if (return_depth) {
            for (int k = 0; k < 9; k++) {
                face_inv_map[bn*image_size*image_size*9 + yi*image_size*9 + xi*9 +k] = face_inv_min[k];
            }
        }
    }
}
...
std::vector<at::Tensor> forward_face_index_map_cuda(
    ...
    const int buffer_size = face_list.size(3);
